...
if ('navigation' in window) {
  ...
  // activeSeason / activeEpisode / activeShowId — optional; passed from the
  // watch page so we can jump to the correct season and mark the playing
  // episode WITHOUT a second async showSeason call (no double-grid bug).
  function wireEpisodes(data, activeSeason, activeEpisode, activeShowId) {
    const seasons = (data.seasons || []).filter(s => s.season_number > 0);
    if (!seasons.length) return;

    const startSeason = activeSeason || seasons[0].season_number;
    const pageBySeason = {};
    ...
    const renderPage = (page) => {
      ...
      // Mark the currently-playing episode (watch page only)
      if (activeShowId && activeEpisode) {
        $$('.episode', grid).forEach(el => {
          if (el.getAttribute('href') === `/watch/tv/${activeShowId}/${currentSeason}/${activeEpisode}`) {
    ...
    // Single call to the correct season — no redundant .click() that causes double load
    showSeason(startSeason);
  }
  ...
  function attachStageAnchor(anchor) {
    ...
    // Mirrors the YouTube / Netflix "scroll to pip" behavior 
