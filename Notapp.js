...
if ('navigation' in window) {
  ...
  async function _resolveAniZipMap(anilistId) {
    ...
    const p = fetch(`https://api.ani.zip/mappings?anilist_id=${anilistId}`)
  ...
  async function _fetchTmdbTv(tmdbId) {
    ...
    const p = fetch(`${TMDB_BASE}/tv/${tmdbId}?api_key=${TMDB_API_KEY}`)
      .then(r => r.ok ? r.json() : null)
      .catch(() => null);
  ...
  async function renderAnimeWatch(anilistId, episodeNum) {
    ...
    if (watchPage) {
      ...
      // TMDB-driven season + episode list (replaces AniList prequel/sequel
      // chain). We resolve the AniList id → TMDB id via api.ani.zip, then ask
      // TMDB for the canonical season layout. Falls back to the flat AniList
      // episode count if the mapping or TMDB call fails.
      (async () => {
        try {
          const aniMap = await _resolveAniZipMap(id);
          const tmdbId = aniMap && aniMap.tmdb;
          if (!tmdbId) return;
          const tv = await _fetchTmdbTv(tmdbId);
          ...
          // Best-guess starting season: use the per-episode mapping from
          // ani.zip if available, otherwise season 1.
          let activeIdx = 0;
          const epMap = aniMap.episodes && aniMap.episodes[String(ep || 1)];
          if (epMap && epMap.seasonNumber != null) {
            const i = seasons.findIndex(s => s.season_number === epMap.seasonNumber);
            if (i >= 0) activeIdx = i;
          }

          const showName = (tv && (tv.name || tv.original_name)) || title;

          const renderPills = () => {
            pillsEl.innerHTML = seasons.map((s, i) => {
              const isActive = i === activeIdx;
              const label = s.name && !/^season\s+\d+$/i.test(s.name) ? s.name : `Season ${s.season_number}`;
              return `<button class="an-season-pill ${isActive ? 'is-active' : ''}" data-i="${i}" title="${html(label)}">
          ...
          const renderTmdbEps = (season) => {
            const count = season.episode_count || 0;
            if (!count) {
              grid.innerHTML = '<p class="an-empty" style="padding:24px;color:var(--text-mute)">No episodes listed for this season.</p>';
              if (subEl) subEl.textContent = `${showName} · S${season.season_number} · upcoming`;
              return;
            }
            grid.innerHTML = Array.from({ length: count }, (_, i) => {
              const n = i + 1;
              const isCurrent = epMap
                && epMap.seasonNumber === season.season_number
                && epMap.episodeNumber === n;
              const playingCls = isCurrent ? ' episode--playing' : '';
              return `
                <a class="episode episode--compact${playingCls}" href="/watch/tv/${tmdbId}/${season.season_number}/${n}" data-link>
                  <div class="episode__numbox">
                    <span class="episode__num">E${n}</span>
                    <span class="episode__playhint"><i class="ri-play-fill"></i></span>
                  </div>
            ...
            if (subEl) subEl.textContent = `${count} episode${count>1?'s':''} · S${season.season_number}${season.name && !/^season\s+\d+$/i.test(season.name) ? ' · ' + season.name : ''}`;
          };

          renderPills();
          renderTmdbEps(seasons[activeIdx]);
