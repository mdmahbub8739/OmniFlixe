window.OMNIFLIX_CONFIG = {
  ...
  // ── Community + monetization links ────────────────────────────────────────
  // Shown inside the server picker footer. Leave an entry empty ('') to hide
  // that chip. `label` is what users see, `url` is where the chip opens.
  LINKS: {
    TELEGRAM: { label: 'Join Telegram',  url: 'https://t.me/your_channel' },
    ADS:      { label: 'Advertise here', url: 'mailto:ads@example.com' },
    MONETIZE: { label: 'Support us',     url: 'https://example.com/support' },
  },
};
